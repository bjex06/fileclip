...
echo "<h1>Delete API Update</h1>";

$deletePhpContent = <<<'PHP'
<?php
/**
 * ユーザー削除API
 */
...
$targetPath = __DIR__ . '/api/users/delete.php';
$result = file_put_contents($targetPath, $deletePhpContent);

if ($result !== false) {
    echo "<p style='color:green'>✅ delete.php を更新しました ({$result} bytes)</p>";
    echo "<p>変更点:</p>";
    echo "<ul>";
    echo "<li>論理削除（is_active=0）→ 物理削除（DELETE）に変更</li>";
    echo "<li>user_id と id の両方を受け付けるように変更</li>";
    echo "<li>super_admin も削除禁止に追加</li>";
    echo "</ul>";
} else {
