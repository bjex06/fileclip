...
try {
    ...
    try {
        ...
        // 削除対象のファイルを取得
        if ($isAdmin) {
            $stmt = $pdo->prepare("SELECT id, storage_path, thumbnail_path, size, created_by FROM files WHERE is_deleted = TRUE");
            $stmt->execute();
        } else {
            $stmt = $pdo->prepare("SELECT id, storage_path, thumbnail_path, size, created_by FROM files WHERE is_deleted = TRUE AND created_by = ?");
            $stmt->execute([$userId]);
        }
